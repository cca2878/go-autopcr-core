...
//
// 对应参考项目中的 PanicError（model/error.py）与 apiclient 的
...
// PanicError 表示致命错误：应中止整条流程（对应参考项目 PanicError）。
...
// 字段设计求解（captcha 为什么移出核心见 docs/architecture.md）。可能为空。
...
// 以保持参考项目"解码失败视为网络异常"的重试语义不变：既有的 errors.As(err, &NetworkError{})
...
// resultCodeUnrecoverable 是服务端表示"本次请求不可恢复"的业务码（参考项目对 203 的处理）。
const resultCodeUnrecoverable = 203

// maintenanceMarker 是维护中错误消息的特征串（参考项目按消息文本判定维护）。
...
// errorhandler 中间件调用本函数升级——与参考项目的分层一致。把它也并进传输层并无实质差别
