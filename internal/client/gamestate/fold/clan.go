...
// clanLike 记下"今日已点赞"（对应参考项目 ClanLikeResponse 的 clan_like_count = 1）——
// 这类响应本身不带状态，折的是"动作已发生"这一事实：不折则同一会话里重跑同一模块会绕过
// 守卫、撞上业务错误码。
func clanLike(s *gamestate.PlayerState, _ protocol.Request, _ any) {
	s.ClanLikeCount = 1
}

// clanInfo 折叠公会 id（对应参考项目 handlers.py:865 的 mgr.clan = self.clan.detail.clan_id）。
