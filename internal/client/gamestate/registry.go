...
// 定位该更新哪条状态。参考项目的 138 个 handler 里有 20 个属于此类，且形态一致——
// story/viewing 不回传读了哪篇（靠 request.story_id）、quest/skip 只回 daily_clear_count
// 不回 quest_id、deck/update 甚至直接把整份编队从请求里抄回。这是协议的固有性质，不是某种
// 实现偏好，故签名一开始就留出 req，免得将来为它翻修全部折叠器。
...
// Registry 把响应分发给折叠器（对应参考项目 handlers 的注册表）。
//
// 它只是机制，不含任何折叠逻辑——具体折叠器住在子包 gamestate/fold 里（分包理由见该包
// 的文档注释）。
