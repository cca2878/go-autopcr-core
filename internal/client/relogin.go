...
// 会话失效的判定与重登策略（对应参考项目 clientpool.SessionErrorHandler + sessionmgr.request）。
//
...
const (
	...
	// resultCodeSessionLost 是参考项目与 6002 并列处理的另一个会话错误码；上游未命名，照搬。
	resultCodeSessionLost = 4
	// resultCodeBadCredential 是假凭据 / 未知账号的正常返回。它的 message'同样'含
	// titleScreenMarker，但同一 access_key 重跑登录必然再次失败（假 access_key 探测
	// 得到的正是这个错误码），故显式豁免：错误照样上抛，只是不做无用的重登。
	resultCodeBadCredential = 107

	// statusSessionInvalid 是 server_error.status 中表示会话失效的取值（参考项目 ex.status == 3）。
	...
	// maxReloginRetries 是"重登后原样重发"的次数上限（参考项目 SESSION_ERROR_MAX_RETRY）。
	maxReloginRetries = 2
)
...
//
// 惰性重登（置位 + 请求前 ensure）复刻参考项目，并'连带清零玩家状态'：登录序列是权威全量
	...
	return func(next transport.Handler) transport.Handler {
		return func(ctx context.Context, req protocol.Request, out any) (protocol.ResponseHeader, error) {
			...
			for attempt := 0; ; attempt++ {
				...
				// 判为断点、模块按策略中止了；抢在发包前重登，调用方全程无感（复刻参考项目
